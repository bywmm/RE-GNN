...
class REGATConv(MessagePassing):
    ...
    #             index, ptr,
    #             size_i, attn_edge, norm) -> Tensor:
    #     alpha = alpha_j + alpha_i
    #     if attn_edge is not None:
    #         attn_edge = attn_edge.reshape(-1, self.heads)
    #         alpha += attn_edge
    #     alpha = F.leaky_relu(alpha, self.negative_slope)
    #     alpha = softmax(alpha, index, ptr, size_i, 1)
    #     self._alpha = alpha
    #     alpha = F.dropout(alpha, p=self.dropout, training=self.training)
    #     return x_j * alpha.unsqueeze(-1)

    def message(self, x_j, ew):
        # print(x_j.shape, ew.shape)
